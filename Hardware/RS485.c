...
uint8_t rs485_rxbuf[rs485_rxbuf_size]={0};//串口单字节接收上位机Modbus指令缓冲�?
uint8_t rs485_txbuf[rs485_txbuf_size]={0};//串口单字节发送上位机Modbus指令缓冲�?
uint8_t buf_485[3]={0};
...
{
	HAL_UARTEx_ReceiveToIdle_IT(huartx, rs485_rxbuf, rs485_rxbuf_size);//485接收初始�?
...
void manualBox485Func03ResponseCallBack(manualBox485_t *_mb485,manualBoxHoldreg_t *_mbHoldreg,
	...
	uint16_t regAdd,regLen,crc;//寄存器起始地址,寄存器单元长度，数据，crc校验�?
	uint32_t poseInt32,poseSpeedInt32,loadInt32,strainInt32;
	uint8_t sizeByte,i=0,j=0;//字节�?
	...
	regLen = MB485_regLen;//寄存器单元长�?
	sizeByte = regLen*2;//字节�?
	...
	_mb485->sendbuf[i++]=(uint8_t)(crc>>8);//CRC16�?
	_mb485->sendbuf[i++]=(uint8_t)crc;//CRC16�?
	//发送数据包 
...
manualBox485_e manualBox485Func10ResponseCallBack(manualBox485_t *_mb485){
	uint16_t regAdd,regLen;//寄存器起始地址,寄存器单元长度，字节数，数据，crc校验�?
	// uint8_t j=0;
	regAdd = (_mb485->rcbuf[2]<<8)+_mb485->rcbuf[3];//读取寄存器起始地址
	regLen = (_mb485->rcbuf[4]<<8)+_mb485->rcbuf[5];//寄存器单元长�?
...
manualBox485_e manualBox485Func05Response(manualBox485_t *_mb485,manualBoxCoil_t *_mbCoil){
	uint16_t regAddr,coilState,crc;//寄存器起始地址，线圈状�?crc校验�?
	uint8_t i=0;
	bool state;
	regAddr = (_mb485->rcbuf[2]<<8)+_mb485->rcbuf[3];//读取寄存器起始地址,即线圈地址
	coilState = (_mb485->rcbuf[4]<<8)+_mb485->rcbuf[5];//读取线圈状�?
	state = (coilState == 0xFF00 ? true:false);//设置对应引脚状�?
	//printf("regAddr:%d,state:%d\r\n",regAddr,state);
	manualBoxCoilUpdate((uint8_t)regAddr,state);
	//发送回应数据包
	_mb485->sendbuf[i++]=_mb485->mainBoardAddr;//ID号：发送本机设备地址
	_mb485->sendbuf[i++]=0x05;//功能�?
	_mb485->sendbuf[i++]=(uint8_t)(regAddr>>8);//线圈地址�?
	_mb485->sendbuf[i++]=(uint8_t)regAddr;//线圈地址�?
