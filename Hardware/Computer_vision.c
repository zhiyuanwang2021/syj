...
#define CMD_IMG     1
    ...
    uart_putbuff(WARE_UART, cmdf, sizeof(cmdf));    //先发送命令

    uart_putbuff(WARE_UART, (uint8_t *)imgaddr, imgsize); //再发送图像

    uart_putbuff(WARE_UART, cmdr, sizeof(cmdr));    //先发送命令
}
...
#define CMD_CCD     2
    ...
    uart_putbuff(WARE_UART, (uint8_t *)ccdaddr, ccdsize); //再发送图像

    uart_putbuff(WARE_UART, cmdr, sizeof(cmdr));    //再发送命令
}
