...
void SetTareMonitor(APP_LAYER_VARIABLE_STRUCT *p); // 传感器值清零检�?
	...
	case SGINAL_PULSE_SIGN:
		...
		case POS_MODE:
			...
			if (AL.complete_state.flag == 1 || stateFlag.DoPESetCtrl == CtrlFALSE)
				hal_output.PWM = 0; // 代表完成此次运动或闭环禁�?禁止输出
			break;
		case LOAD_MODE:
...
/**
 * @brief 处理限位保护逻辑，包括硬件限位和软限位保�?
 * 
 * 此函数会检查硬件和软件的上下限位触发情况，根据不同的限位触发条件和当前的PWM保护值，
 * 禁用相应的PWM输出，以确保系统的安全运行�?
 * 
 * 注意�?.保护函数有实际效果的前提是：在应用层显示的位置方向与实际的位置方向相同！
 * 		 2.本保护逻辑都是基于闭环控制的输出PWM或AO值，这里特指没有进入输出映射前的值�?
 * 		   因为在输出映射后会被输出符号改变PWM输出的方向，不能以改变后的PWM值来判断�?
 * 		   所以这里的PWM保护值是没有经过输出映射的�?
 * 
 * 保护流程：①->�?>�?
 * 			①Emergency_Monitor()函数在loopctrl最开始被调用，检测硬件限位触发情�?
 * 			②sftLimitMonitor()函数在数据输入映射后被调用，检测软件限位触发情�?
 * 			③limitProtectProcess()函数在loopctrl最后被调用，根据限位触发情况禁用相关输�?
 * 
 * @param �?
...
void limitProtectProcess(void){
	...
	//需要改成针对不同模式触发的软限位进行不同的处理，方法添加软限位触发的标志种�?例如：emergencySftUpperPos
