...
void normalPosGeneratorInit(uint8_t movectrl){
	switch(movectrl)
	{
		case POS_MODE:
			...
			{
				pg.st = pose.orig + speedPose.filter*0.001;
				pg.vt = speedPose.filter;	
			}
...
float PosPos_LoopCtrl(float setpos,float nowpos)
{
	...
	static float outI=0;
	static float lastpose=0,lasterr=0;
	err = setpos - nowpos;
...
float posPwmCalcu(const float _input,float *_out,const uint16_t _time){
	...
	if(SenData[ch0Pose].LinPoint == 0)
		...
		*_out = (int64_t)(_input * 100000000.0 / (AL.posCtrl.NominalSensitive * servoParam.ResolutionGearRate) * _compensateTime);
	else{
		...
		*_out = (int64_t)(servoCode*100000000.0 / ( servoParam.ResolutionGearRate) * _compensateTime);
	}
#ifdef ADRC_CTRL_POS
...
void pwmPosLsmCalcu(void)
{
	...
	if(SenData[ch0Pose].LinPoint == 0)
		servoOutput = (int64_t)(AL.posCtrl.posLoopCtrlOut * 100000000.0 / (AL.posCtrl.NominalSensitive * AL.posCtrl.sign * servoParam.ResolutionGearRate) * 1000.0f / tim6us);
	else{
		sensorCalibrate.antiMultipointCalibrate(AL.posCtrl.posLoopCtrlOut,ch0Pose,&SenData[ch0Pose],&servoCode);
		servoOutput = (int64_t)(servoCode*100000000.0 / (AL.posCtrl.sign * servoParam.ResolutionGearRate) * 1000.0f / tim6us);
	}
#ifdef ADRC_CTRL_POS
	//  if(fabs(pg.vm) >= 0.005 && AL.creepFlag == 0)
...
float loadPID(const float _target,const float _real,const float _maxRange,float *_vm,
	...
	*_count = 0;
	...
	if(fabs(_target - pgLoad.s0) > fabs(AL.loadCtrl.setPos - pgLoad.s0)*0.2 && fabs(_real - AL.loadCtrl.setPos) > fabs(AL.loadCtrl.setPos - pgLoad.s0)*0.2
		&& fabs(AL.loadCtrl.setPos - pgLoad.s0) > AL.loadCtrl.NominalValue*1000.0f*0.1)
		AL.loadCtrl.detaLoopCtrlOut += pgLoad.vt*0.02 / lsRigidity.kFilter;//lsRigidity.kFilter
	//limit
	AL.loadCtrl.detaLoopCtrlOut = constraint(AL.loadCtrl.detaLoopCtrlOut,10,-10);
	//_loadRecord = pose.orig;
	_loadRecord = *_out;
	*_vm = AL.loadCtrl.detaLoopCtrlOut / 0.02;
	}
...
void pwmLoadLsmCalcu(void)
{
	...
	if(SenData[ch0Pose].LinPoint == 0)
		servoOutput = (int64_t)(AL.loadCtrl.speedLoopCtrlOut * 100000000.0 / (AL.posCtrl.NominalSensitive * AL.posCtrl.sign * servoParam.ResolutionGearRate) * 1000.0f / tim6us);
	else{
...
void pwmExtenCalcu(void)
{
	...
	if(SenData[ch0Pose].LinPoint == 0)
		servoOutput = (int64_t)(AL.ext1Ctrl.speedLoopCtrlOut * 100000000.0 / (AL.posCtrl.NominalSensitive * AL.posCtrl.sign * 0.4) * 1000.0f / tim6us);
	else{
	...
	// else
		z2 = 0;
#endif
	svPWM.ext = (servoOutput - z2)/100000000.0;
	AL.lesoU = svPWM.ext * (AL.posCtrl.NominalSensitive * AL.posCtrl.sign * 0.4);
...
float extenPID(const float _target,const float _real,float *_vm,
	...
	*_count = 0;
	...
	_extenRecord = *_out;
	*_vm = AL.ext1Ctrl.detaLoopCtrlOut / 0.02;
	}
