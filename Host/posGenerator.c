...
uint32_t DynCycle_normal_t3calcu(PGDYN* x)
{
	...
	switch(x->waveform)
	{
		case COSINE:
			...
			t3 = acos(x->destination/x->amplitude)/(2.0f*PI*x->frequency)*1000.0f;//结束时间 ms 
...
uint32_t DynCycle_modify_t3calcu(PGDYN* x)
{
...
#endif
	{
		switch(x->waveform)
		{
			case COSINE:
					if(x->amplitude > 1)
					t3 = (uint32_t)(acos(1/x->amplitude)/(2*PI*x->frequency)*1000);
					else if(x->amplitude<1)
					t3 = (uint32_t)(acos(x->amplitude)/(2*PI)*1000);
					else
					t3 = 0;
				break;
			case TRIANGLE:
...
uint32_t DynCycle_modify_t5calcu(PGDYN* x)
{
	...
	switch(x->waveform)
	{
		case COSINE:
			...
			t5 = acos(x->destination/x->amplitude)/(2.0f*PI*x->frequency)*1000.0f;//结束时间 ms 
...
float AmplitudeMinDistance_calcu(PGDYN* x)
{
	...
	{
		case COSINE:
			ret = (1.0f-cos(2.0f*PI*x->frequency*0.001f))*x->amplitude;
		break;
		case TRIANGLE:
