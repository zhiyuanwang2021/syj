...
/* controller run as server */
int32_t utc_tcps(uint8_t sn, uint8_t* revbuf,uint16_t *revsieze,uint8_t* sendbuf,uint16_t* sendsize, uint16_t port);

/* controller run as client */
int32_t utc_tcpc(uint8_t sn,uint8_t* destip, uint16_t destport);
//int32_t utc_tcpc(uint8_t sn,UTC_ETH* EthStruct, uint8_t* destip, uint16_t destport);
