...
uint16_t modbus_crc_compute(const uint8_t *buf, uint16_t buf_len) {
    ...
    for (uint16_t index = 0; index < buf_len; ++index) {
        uint8_t num = (uint8_t) (modbus16 & UINT32_MAX);
        modbus16 = (uint16_t) (((uint32_t) modbus16 >> 8) ^ crc_tb[(num ^ buf[index]) & UINT8_MAX]);
    }
    return modbus16;
}

uint16_t pack_bool_array(uint8_t *dst, const bool *data, const uint16_t data_len) {
    uint8_t retLen = 0;
    uint8_t curDat = 0;
    uint8_t bitIdx = 0;
